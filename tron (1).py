...
class TronPickBot:
    ...
    def solve_captcha(self, action="captcha"):
        try:
            ...
            if res.get('status') != 1:
                add_log(f"Captcha send failed: {res.get('request')}", "red", self.account_name)
                return None
    ...
    def login(self):
        try:
            add_log("Connecting to TronPick Server...", "cyan", self.account_name)
            ...
            try:
                ...
                add_log("Initial Connection Slow, Retrying...", "yellow", self.account_name)
            ...
            if res.get('ret') == 1:
                ...
                add_log("Login Successful!", "green", self.account_name)
                return True, "Success"
            add_log(f"Login Failed: {res.get('mes')}", "red", self.account_name)
    ...
    def claim(self):
        try:
            ...
            if res.get('ret') == 1:
                add_log(f"CLAIM SUCCESS: {res.get('mes')}", "bold green", self.account_name)
            else:
                add_log(f"CLAIM FAILED: {res.get('mes')}", "bold red", self.account_name)
            self.update_info()
        except:
            add_log("Claim Server Timeout", "red", self.account_name)

def account_worker(bot, stop_event):
    """Har bir account uchun alohida thread - vaqt hisoblagichi to'g'ri"""
    while not stop_event.is_set():
        try:
            ...
            # Agar vaqt 0 bo'lsa, claim qilish
            if bot.claim_time_remaining <= 0:
                # Update qilamiz (serverdan yangi vaqt olish uchun)
                bot.update_info()
                
                # Agar hali ham 0 bo'lsa, claim qilamiz
                if bot.claim_time_remaining <= 0:
                    bot.claim()
                    bot.update_info()
...
def build_dashboard(accounts):
    """Multi-account dashboard - Level olib tashlandi"""
    ...
    stats_panel = Panel(stats_table, title="[bold white]📊 TRX STATS - MULTI ACCOUNT[/]", border_style="bright_blue")
...
def show_menu(accounts_config):
    ...
    console.print(Panel.fit(
        "[bold cyan]⚡ TRONPICK MULTI-ACCOUNT BOT ⚡[/]",
